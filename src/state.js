...
const Game = {
  ...
  // ---------- 設置は1六角。**武器の大きさという概念は無い** ----------
  //
  //   **ユーザー 2026-09-23**
  //   > 「武器を置いたら謎の四角が出てきます、おそらくこれは武器の2マス要求とかを
  //   >   要望した時の名残ですね。**ハニカム形式に移行した**のと、**壁貫通を無くした**こと、
  //   >   **置ける場所を壁沿いのみにした**ことで、**武器の大きさ概念は必要なくなりました**、
  //   >   こちらも1マスでいいでしょう」
  //
  //   広さで重みを付けていたのは、壁の中に埋め込めた頃の名残。
  //   いまは壁を弾が抜けないので、**置ける場所が壁沿いに限られること自体が重み**になっている。
  //   `footTiles` は「そのセル1つ」を返すだけにして、呼び出し側は変えない
  //   （消すと設置・移動・復元・描画の4か所に同じ形の変更が散る）
  //
  //   **武器ごとに要る広さが違う。**（ユーザー 2026-09-22）
  //   回さない（形は固定）。強い武器ほど広いので、置ける場所が限られる

  // その武器を (c,r) に置いたとき、実際に埋まる**六角セル**
  //   **どの武器も1セル。** 引数の `def` は使わない（呼び出し側の形を変えないために残す）
  footTiles(def, c, r) {
    return [{ c, r }];
  },
