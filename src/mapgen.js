...
//
//   **3が肝。** 経路探索（BFS）・押し合い（Crowd）・当たり判定は
//   ぜんぶタイル前提のまま動く。絵は vec（折れ線と幅・六角）を見て描く。
//   **設置は六角セル**（2026-09-23 のユーザー指示で、当初の「設置はタイルのまま」から
//   置き換わった。stage.hexBuildable が vec の六角から決める）。
//
...
const MapGen = {
  ...
  //   **【その後】後半の盤は広げた。**（ユーザー 2026-09-22「ボス章や後半ステージ、
  //   ラスボスは広いマップ＋複雑な地形＋分断された敵の襲撃を意識して」）
  //   ここの COLS/ROWS は既定（第1〜11章）の大きさで、深い章は BAL.boardByDepth が
  //   19×27〜27×37 に広げる。画面に収まらないぶんは Render.canPan で見渡す
  COLS: 15,
  ROWS: 21,
  ...
  tagZones(hexes, rnd, d) {
    // **一度切って、凡例つきで戻した（ver 0922k・BAL.zoneOn = true）。**
    //   切った理由（ユーザー 2026-09-22）：
    //   > 「ハニカムマップに緑やピンクの謎のマス目がありますが、
    //   >   装飾なら混乱を招くため一旦やめておきましょう」
    //   装飾ではなく仕掛け（減速＝敵が遅くなる／加速＝速くなる）だったが、
    //   **見て意味が分からない時点で仕掛けとして成立していなかった。**
    //   盤の隅に凡例（Render.zoneLegend）を出して戻し、ユーザーも「残します」と返答。
    //   BAL.zoneOn を false にすれば消える（ZONE の mud/slope は内部名。表示は減速/加速）
  ...
  make(seed, d, shape) {
    ...
    return {
      ...
      zone: this._zone,                 // タイルごとの仕掛け（0=なし 1=減速 2=加速）
      shape,
      seed,
    };
  },
