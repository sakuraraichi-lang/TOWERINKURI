...
// 設置に使うマス（foot）
//
//   **武器ごとに、置くのに要る広さを変える。**（ユーザー 2026-09-22）
//   > 「武器毎に設置に必要なマスを変えて、設置に重みをつけてもいいと思います。
//   >   例えばスナイパーであれば縦もしくは横に二マス使用する、などですね。
//   >   テトリスみたいに回転させる必要性はないと思います、設置マスを回転させながら
//   >   打ちたい場所を360度選べるならあまり設置に対する重みになってません。
//   >   これにてガトリングよりも強い報酬武器やパック産武器も、癖があるから
//   >   置き方を選ぶなどの必要性が出ます」
//
//   **回さない。** 形は武器ごとに固定。回せるうえに360度撃てるなら、
//   広さが制約にならない。
//
//   `foot` は基準マスからの [列, 行] のずれ。無ければ1マス。
//   **強い武器ほど広い**ので、置ける場所が限られる＝それ自体が代償になる。
//   盤の地面はおよそ200マスだが、通路に面した良い場所はその一部しかない
// ---------------------------------------------------------------
const FOOT = {
  gatling:  [[0, 0]],                          // 初期武器。どこにでも置ける
  tesla:    [[0, 0]],                          // 小さいコイル
  cryo:     [[0, 0]],                          // 支援。置き場所を選ばせない
  sniper:   [[0, 0], [1, 0]],                  // 長い砲身。横に2
  missile:  [[0, 0], [1, 0]],                  // 発射架。横に2
  flame:    [[0, 0], [1, 0]],                  // 燃料タンクぶん横に2
  gas:      [[0, 0], [1, 0]],                  // ボンベぶん横に2
  shuriken: [[0, 0], [1, 0]],                  // 射出機。横に2
  tentacle: [[0, 0], [0, 1]],                  // 縦に2。他と向きが違う＝置き場所の癖
  katana:   [[0, 0], [0, 1]],                  // 一番強い。縦に2＝置き場所の癖が一番きつい
  bubble:   [[0, 0], [0, 1]],                  // 大がかりな散布機。縦に2
  mortar:   [[0, 0], [0, 1]],                  // 重砲。縦に2
};

//   **2×2 は使わない。**（2026-09-22 実測）
//   通路を六角で焼いているので、地面は四角くまとまっていない。
//   2×2 を要る武器は置ける場所がほとんど無く、通しが
//   `2→5→9→4→4→4` のように崩れた（3シードとも）。
//   ユーザーの例も「縦もしくは横に二マス」なので、**最大2マスに揃える**。
//   重みは「広さ」ではなく**向き**で付ける：
//   横に2（狙撃・ミサイル・火炎・毒ガス・手裏剣）と 縦に2（触手・刀・泡・迫撃）は、
//   同じ地面でも置ける場所が違う
...
const WEAPONS = {
  ...
  gatling: {
    ...
    base: baseStats({ arc: 0.34, dmg: 6, rate: 5.5, range: 195, spread: 0.07, speed: 640, bulletR: 2.6, turn: 9, pierce: 1 }),
    fire(w, run) {
      for (let i = 0; i < w.n; i++) {
  ...
  tentacle: {
    ...
    base: baseStats({ arc: 0.34, dmg: 26, rate: 1.2, range: 210, count: 3, knock: 105, knockDur: 1.3, turn: 9 }),
    fire(w, run) {
      const t = w.target;
      if (!t) return;
...
// 設置マスを各武器に配る。**ここで一括**にしておけば、定義とズレない
for (const id of Object.keys(WEAPONS)) WEAPONS[id].foot = FOOT[id] || [[0, 0]];
