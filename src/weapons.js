...
// ---------------------------------------------------------------
// **設置は六角（ハニカム）。**（ユーザー 2026-09-23）
//   > 「ハニカムの壁の中のデザインをハニカムにしろという指示、これは同じく
//   >   **武器設置時の置ける場所もハニカムにする**という、システム面での変更でもあります」
//
//   値は `MapGen.hexNbr` の**向きの番号**。`null` なら1セルだけ。
//     0 = 上 ／ 1 = 下 ／ 2,3 = 左 ／ 4,5 = 右
//   **回さない（形は固定）。** 強い武器ほど広いので、置ける場所が限られる。
//   **向きで癖を付ける**のは四角のときと同じ狙い：
//   「下に2」と「右に2」は、同じ壁でも置ける場所が違う
const FOOT = {
  gatling:  null,   // 初期武器。どこにでも置ける
  tesla:    null,   // 小さいコイル
  cryo:     null,   // 支援。置き場所を選ばせない
  sniper:   4,      // 長い砲身。右に2
  missile:  4,      // 発射架。右に2
  flame:    4,      // 燃料タンクぶん右に2
  gas:      4,      // ボンベぶん右に2
  shuriken: 4,      // 射出機。右に2
  tentacle: 1,      // 下に2。他と向きが違う＝置き場所の癖
  katana:   1,      // 一番強い。下に2＝置き場所の癖が一番きつい
  bubble:   1,      // 大がかりな散布機。下に2
  mortar:   1,      // 重砲。下に2
};
...
//   `foot` は六角の隣の向き（`MapGen.hexNbr` の番号）。null なら1セル
for (const id of Object.keys(WEAPONS)) {
  WEAPONS[id].foot = (FOOT[id] === undefined) ? null : FOOT[id];
}
