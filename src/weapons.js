...
// ---------------------------------------------------------------
// **【撤去 2026-09-23・ユーザー指示】武器の大きさという概念は無い。**
//
//   > 「武器を置いたら謎の四角が出てきます、おそらくこれは
//   >   **武器の2マス要求**とかを要望した時の名残ですね。
//   >   **ハニカム形式に移行した**のと、**壁貫通を無くした**こと、
//   >   **置ける場所を壁沿いのみにした**ことで、
//   >   **武器の大きさ概念は必要なくなりました**、こちらも1マスでいいでしょう」
//
//   ここには `FOOT`（武器ごとに要る設置マス）があった。
//   置き場所の重みを「広さ」で付けるためのもので、
//   **壁の中に武器を埋め込めた頃の名残**だった。
//   いまは弾が壁を抜けないので（0922q）、**置ける場所が壁沿いに限られること自体が重み**。
//   広さで二重に縛る理由が無い。
//
//   **どの武器も1六角。** `Game.footTiles` はそのセル1つだけを返す
// ---------------------------------------------------------------
...
// **設置マスは配らない。**（2026-09-23・ユーザー指示で武器の大きさ概念を撤去）
//   どの武器も1六角。`Game.footTiles` がそのセル1つだけを返す
