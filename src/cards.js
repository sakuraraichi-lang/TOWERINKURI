...
const CARDS = {
  ...
  snp_scope: C({ id: 'snp_scope', kind: 'mod', weapon: 'sniper', rarity: 'common', maxStack: 5,
    ...
    apply(run) { const w = run.wp('sniper'); if (w) { w.s.range *= Game.rk(1.35); w.s.dmg *= Game.rk(1.10); } } }),
  ...
  msl_warhead: C({ id: 'msl_warhead', kind: 'mod', weapon: 'missile', rarity: 'common', maxStack: 5,
    ...
    apply(run) { const w = run.wp('missile'); if (w) { w.s.splash *= Game.rk(1.35); w.s.dmg *= Game.rk(1.22); } } }),
  ...
  msl_nuke: C({ id: 'msl_nuke', kind: 'mod', weapon: 'missile', rarity: 'legendary', maxStack: 1,
    ...
    apply(run) { const w = run.wp('missile'); if (w) { w.s.dmg *= Game.rk(3.2); w.s.splash *= Game.rk(2.4); w.s.rate *= Game.rk(0.55); } } }),
  ...
  flm_wide: C({ id: 'flm_wide', kind: 'mod', weapon: 'flame', rarity: 'rare', maxStack: 3,
    ...
    apply(run) { const w = run.wp('flame'); if (w) { w.s.cone *= Game.rk(1.45); w.s.range *= Game.rk(1.18); } } }),
  ...
  cry_wide: C({ id: 'cry_wide', kind: 'mod', weapon: 'cryo', rarity: 'rare', maxStack: 3,
    ...
    apply(run) { const w = run.wp('cryo'); if (w) { w.s.range *= Game.rk(1.35); w.s.rate *= Game.rk(1.2); } } }),
  ...
  tnt_long: C({ id: 'tnt_long', kind: 'mod', weapon: 'tentacle', rarity: 'rare', maxStack: 3,
    ...
    apply(run) { const w = run.wp('tentacle'); if (w) { w.s.range *= Game.rk(1.35); w.s.knock *= Game.rk(1.35); } } }),
  ...
  bbl_big: C({ id: 'bbl_big', kind: 'mod', weapon: 'bubble', rarity: 'common', maxStack: 5,
    ...
    apply(run) { const w = run.wp('bubble'); if (w) { w.s.stunDur *= Game.rk(1.35); w.s.splash *= Game.rk(1.25); } } }),
  ...
  mtr_wide: C({ id: 'mtr_wide', kind: 'mod', weapon: 'mortar', rarity: 'rare', maxStack: 3,
    ...
    apply(run) { const w = run.wp('mortar'); if (w) { w.s.splash *= Game.rk(1.40); w.s.range *= Game.rk(1.18); } } }),
  ...
  gen_boost: C({ id: 'gen_boost', kind: 'generic', rarity: 'rare', maxStack: 3,
    ...
    apply(run) { for (const w of run.units) { w.s.rate *= Game.rk(1.15); w.s.range *= Game.rk(1.12); } } }),
  // ============ 遺物（永続パッシブ）============
  //
