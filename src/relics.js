...
// ---------------------------------------------------------------
// ---------------------------------------------------------------
// **転生の格。**（2026-09-25・プレイヤーの感想 → ユーザー「経済カーブが破綻しています」）
//   > 「2章転生を繰り返し続けると後ろの章がクリアできる」
//   > 「×どんどん強くなっていくタワーディフェンスゲーム ○『放置で1章2章クリアという広告』を見てパックを剥くゲーム」
//   前は、遺物の上限・パックの種類の解放・自動化の解放を**転生した回数**で開けていた。
//   回数は第2章で転生しても1増えるので、**浅い転生を繰り返すだけで上限と解放を買えた**。
//   格は**転生したときの到達章（legacyDeep）**から決める。到達章はいちばん深く行った章なので、
//   浅いところで何回転生しても上がらない。転生していなければ 0、したら最低 1
...
const Relic = {
  ...
  mods(perm) {
    ...
    for (const id of RELIC_IDS) {
      ...
      //   同じ扱いにはできない
      const capMul = 1 + 0.6 * prestigeRank(perm);      // 転生の格で開く（回数だと浅い転生で買えた・2026-09-25）
