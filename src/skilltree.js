...
const SKILLS = [
  ...
  ...chain({ id: 'units', gkey: 'units', icon: '⛁', group: '拠点',
    //   **【試して戻した 2026-09-22】24段（天井30基）にはしない。**
    //   湧き口の数を敵の量に効かせる（BAL.waveCountPerHole）と第30章に36基要るので、
    //   天井を 20 → 30 に上げてみたが、**通しが成立しなかった**：
    //     24段・値上がり×9   … 第26章・20基で6周足踏み（クリアできない →
    //                          枠が買えない → クリアできない、の膠着）
    //     24段・値上がり×6.5 … 第25章・22基で7周足踏み、292分
    //   **そもそも 20基はユーザーが決めた数字**（「最終的に上限20基で良いのですが、
    //   それはラスボス時にようやく到達できるくらいの窮屈さがよく、
    //   クリア想定は14基〜くらいが良いです」2026-09-22）。
    //   上げるならユーザーの判断が要る。勝手に動かさない
